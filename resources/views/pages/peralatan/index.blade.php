                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Kode Peralatan</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($peralatan as $item)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $loop->iteration + $peralatan->firstItem() - 1 }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-800 max-w-xs whitespace-normal break-words">{{ $item->kode_peralatan }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                                        ...
                                        @else
                                            <form action="{{ route('peralatan.kirimApi', $item->id) }}" method="POST">
                                                @csrf
                                                <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded text-xs cursor-pointer">
                                                    Kirim
                                                </button>
                                            </form>
                                        @endif
