...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <div>
        ...
        <div class="flex items-center space-x-2 mt-1">
            ...
            <button type="button" class="manage-options-btn flex-shrink-0 px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700" data-type="status_pemeliharaan" data-title="Status Pemeliharaan" data-target-dropdown="#status">
                Kelola
            </button>
            @endif
        </div>
...
<div class="flex items-center justify-end mt-6">
    <a href="{{ route('pemeliharaan.index') }}" class="text-gray-300 hover:text-white mr-4">Batal</a>
...
<div id="confirmation-modal" class="fixed inset-0 bg-gray-600 bg-opacity-75 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-gray-800 text-white">
        <div class="mt-3">
            <h3 class="text-lg leading-6 font-medium text-white text-center mb-4">Konfirmasi Data</h3>
            <div id="confirmation-summary" class="mt-2 px-7 py-3 text-sm text-gray-300 max-h-96 overflow-y-auto">
                {{-- Ringkasan data akan ditampilkan di sini oleh JavaScript --}}
            </div>
            <div class="items-center px-4 py-3 mt-4 flex justify-end space-x-4">
                <button id="cancel-confirmation-btn" class="px-4 py-2 bg-gray-600 text-white text-base font-medium rounded-md shadow-sm hover:bg-gray-700 focus:outline-none">
                    Batal
                </button>
...
<div id="options-modal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-gray-800 text-white">
        <div class="mt-3 text-center">
            <h3 id="modal-title" class="text-lg leading-6 font-medium text-white">Kelola Pilihan</h3>
            <div class="mt-4 flex space-x-2">
                <input type="text" id="new-option-value" placeholder="Nama Pilihan Baru" class="form-input rounded-md shadow-sm block w-full bg-gray-700 text-white">
            ...
            <div class="items-center px-4 py-3 mt-4">
                <button id="close-modal-btn" class="px-4 py-2 bg-gray-600 text-white text-base font-medium rounded-md w-full shadow-sm hover:bg-gray-700">
                    Tutup
                </button>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (openConfirmationBtn) {
        ...
        openConfirmationBtn.addEventListener('click', function() {
            ...
            for (const [key, value] of formData.entries()) {
                ...
                summaryHtml += `<dt class="font-semibold text-gray-400">${label}</dt><dd class="mb-2 text-gray-200">${displayValue}</dd>`;
            }
            summaryHtml += '</dl>';
    ...
    async function loadOptions() {
        optionsListContainer.innerHTML = '<p class="text-gray-400">Memuat...</p>';
        try {
    ...
    function renderOptions(options) {
        ...
        if (options.length === 0) {
            optionsListContainer.innerHTML = '<p class="text-gray-400">Belum ada pilihan.</p>';
            return;
        }
        const list = document.createElement('ul');
        list.className = 'divide-y divide-gray-600';
        options.forEach(option => {
            const listItem = document.createElement('li');
            listItem.className = 'p-2 flex justify-between items-center';
            listItem.innerHTML = `<span class="text-gray-200">${option.value}</span><button data-id="${option.id}" class="delete-option-btn text-red-500 hover:text-red-700 text-sm">Hapus</button>`;
